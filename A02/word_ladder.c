...
static hash_table_t *hash_table_create(void)
{
  ...
  hash_table->hash_table_size = DEFAULT;
  hash_table->number_of_entries = DEFAULT;
  hash_table->number_of_edges = DEFAULT;
...
static void hash_table_grow(hash_table_t *hash_table)
{
  ...
  for (unsigned int i = 0; i < hash_table->hash_table_size; i++)
  {
    ...
    while (node != NULL)
    {
      find_word(hash_table, node->word, 1);
      node = node->next;
    }
  }
...
static hash_table_node_t *find_representative(hash_table_node_t *node)
{
  ...
  hash_table_node_t *representative = node;
...
static void add_edge(hash_table_t *hash_table,hash_table_node_t *from,const char *word)
{
  ...
  hash_table_node_t *from_representative = find_representative(from);
  hash_table_node_t *to_representative = find_representative(to);
  
  if (from_representative != to_representative)
  {
    adjacency_node_t *link = (adjacency_node_t *)malloc(sizeof(adjacency_node_t));
    link->vertex = to;
    link->next = from->head;
    from->head = link;
